...
def check_option(object, client):
    # 按格式解码，确定帧数和分辨率
    info = struct.unpack('lll', client.recv(12))
    if info[0] > 888:
...
def RT_Image(object, client, D_addr):
    ...
    while (1):
        ...
        try:
            # 按照相应的格式进行打包发送图片
            client.send(
                struct.pack("lll", len(object.img_data), object.resolution[0], object.resolution[1]) + object.img_data)
        except:
            camera.release()  # 释放资源
            return

def RT_Image2(object, client, D_addr):
    ...
    while (1):
        ...
        object.img = cv2.resize(object.img, object.resolution)  # 按要求调整图像大小(resolution必须为元组)
        _, img_encode = cv2.imencode('.jpg', object.img, img_param)  # 按格式生成图片
        #print(type(img_encode))     #<class 'numpy.ndarray'>
        img_code = numpy.array(img_encode)  # 转换成矩阵
        #object.img_data = img_code.tostring()  # 生成相应的字符串
        #protocol
        img = ImgData_pb2.ImgData()
        img.msg_type = 1
        img.imgdata = img_code.tostring()


        try:
            # 按照相应的格式进行打包发送图片

            #client.send(img_encode )
            i = img.SerializeToString()
            print(len(i))
            # client.send(i)
...
if __name__ == '__main__':
    print(struct.pack("lll", 903, 640, 480))
    camera = Carame_Accept_Object()
    while (1):

        client, D_addr = camera.server.accept()
        clientThread = threading.Thread(None, target=RT_Image2, args=(camera, client, D_addr,))
=======
#!/usr/bin/python
#-*-coding:utf-8 -x-
import socket
...
if __name__ == '__main__':
    ...
    while (1):
        ...
        client, D_addr = camera.server.accept()
        clientThread = threading.Thread(None, target=RT_Image2, args=(camera, client, D_addr,))
>>>>>>> 624e2da48b0cff1bf680cd433bc942e3a8ba818b
        clientThread.start()